...
class PlantDiagnosticGraph:
    ...
    def _prepare_synthesis_context(self, state: PlantCareState) -> Tuple[Optional[str], Optional[str], str]:
        ...
        # Branch 3: Missing Substrate (Gate 1b - only when required by intent like FEEDING_CARE)
        elif "substrate" in missing_slots:
            clinical_data_summary = {
        ...
        # Branch 6: User Intent is UNSPECIFIED or Onboarding (Information registered, prompt user for goal)
            ...
            if not substrate_data:
                ...
                clinical_data_summary = {
                    "situation": f"کاربر فقط گیاه {plant_desc} را معرفی کرده است و هدف یا مشکل خود را هنوز نگفته است.",
                    "plant": plant_desc,
                    "user_message": state.get("user_message", ""),
                }
                llm_instruction = (
                    f"کاربر فقط گفته که گیاه «{plant_desc}» دارد و هنوز هیچ نیاز یا مشکلی را مشخص نکرده است. "
                    f"دستورالعمل حیاتی و اکید: از هرگونه اجبار یا اولویت دادن به نوع خاک و بستر اکیداً خودداری کنید! "
                    f"به کاربر با صمیمیت و اشتیاق خوش‌آمد بگویید و توضیح دهید که مشتاقید پرونده این گیاه را در «باغچه دیجیتال» او تشکیل دهید. "
                    f"تمرکز اصلی پیام شما باید پرسیدن وضعیت فعلی گیاه و هدف کاربر باشد: "
                    f"بپرسید حال و هوای گیاه چطور است؟ آیا مشکلی مانند زردی، پژمردگی یا لکه دارد، یا برای شرایط نگهداری، نور، آبیاری و تغذیه راهنمایی می‌خواهد؟ "
                    f"در انتها به شکل کاملاً اختیاری و گذرا اشاره کنید که در صورتی که بستر کاشت آن را هم می‌داند می‌تواند بفرماید تا پرونده باغچه‌اش دقیق‌تر شود. "
                    f"اکیداً گزینه‌های متعدد خاکی را قطار نکنید و لحن بازجویی ایجاد نکنید."
                )
                fallback_response = (
                    f"سلام! چقدر عالی، **{plant_desc}** یکی از گیاهان فوق‌العاده و محبوب است. 🌿\n\n"
                    f"خیلی خوشحالم که می‌تونم در نگهداری و سلامت این گیاه همراهتون باشم. برای اینکه پرونده این گیاه رو در **«باغچه دیجیتال شما»** تشکیل بدیم و متناسب با نیازتون راهنماییتون کنم:\n\n"
                    f"🌱 **الان مونسترای شما در چه وضعیتیه؟**\n"
                    f"- آیا مشکل خاصی مثل زردی برگ، لکه یا پژمردگی مشاهده کردید؟\n"
                    f"- یا مایلید درباره نور، نحوه آبیاری و برنامه غذایی آن راهنمایی دریافت کنید؟\n\n"
                    f"*(ضمناً اگر نوع خاکی که در آن کاشته شده را هم می‌دانید خوشحال می‌شوم بفرمایید، ولی در غیر این صورت اولویت با نیاز و سوال شماست!)*"
                )
            else:
                clinical_data_summary = {
